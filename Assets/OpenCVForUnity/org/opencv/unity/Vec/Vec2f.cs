...
namespace OpenCVForUnity.UnityUtils
{
    ...
    public struct Vec2f : IComparable, IComparable<Vec2f>, IEquatable<Vec2f>, IStructuralComparable, IStructuralEquatable
    {
        ...
        // Implicit conversion to ValueTuple<float, float>
        public static implicit operator (float, float)(in Vec2f vec)
        {
            return (vec.Item1, vec.Item2);
        }

        // Implicit conversion from ValueTuple<float, float>
        public static implicit operator Vec2f(in (float, float) valueTuple)
        {
